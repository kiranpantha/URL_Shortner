...
class lilURL
{
	...
	function add_url($url)
	{
		...
		{
			$id = date("U");
			$id=substr(md5($id*12321),0,7);
			///
			$newid='';
				for($count=0;$count<7;$count++){
				  if(rand(0,500)%2!=0)
				  {
					  $newid=$newid.strtoupper(substr($id,$count,1));
				  }
				  else
				  {
					   $newid=$newid.substr($id,$count,1);  
				  }
								
				}
			//
			$q = 'INSERT INTO '.URL_TABLE.' (id, url, date) VALUES ("'.$newid.'", "'.$url.'", NOW())';

			return mysql_query($q);
		}
	}
   //for special purpose
   function add_url_spe($url,$id)
	{
